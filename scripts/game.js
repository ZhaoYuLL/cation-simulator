...
let CTX_level;
const _rgba = (x) => {
  return x[0] * 256 ** 3 + x[1] * 256 ** 2 + x[2] * 256 + x[3];
};
...
var BGRealmSketch;
var DRAWEQUIS;
var BGRealm = function (sketch) {
  // contour lines, information, bg color
  var canvas;
  ...
  sketch.draw = function () {
    if (gameStatus == "playing") {
      ...
      canvas.elt
        .getContext("2d")
        .drawImage(
          document.querySelector("#equis-img"),
          pX(0),
          pY(CTX_level.h),
          sX(CTX_level.w),
          -1 * sY(CTX_level.h)
...
var ObstacleRealm = function (sketch) {
  // point charges, coins, rectangular walls, winbox
  var canvas;
  sketch.setup = function () {
    canvas = sketch.createCanvas(CTX_htmlWIDTH, CTX_htmlHEIGHT);
    canvas.parent("canvas-container");
...
var PlayerRealm = function (sketch) {
  // player charge, mouse, controls
  var canvas;
  ...
  sketch.draw = function () {
    if (gameStatus == "playing" && CTX_level && CTX_playerCharge) {
      ...
      var t = 1 / 150;
      CTX_playerCharge.applyForce(
        CTX_level.FeAtPoint(coords.x, coords.y, CTX_playerCharge.q)
      );
      ...
      var actualPoints =
        ...
        parseInt(
          Math.max(
            0,
            SAVE_DATA[currentLevel].maxtime -
              (new Date().valueOf() - startTime) / 1000
          )
        ) *
          10;
      document.querySelector("#points").innerHTML = "Points: " + actualPoints;
      document.querySelector("#potometer").innerHTML =
        "+" + CTX_level.VAtPoint(coords.x, coords.y).toFixed(2) + "V";
      document.querySelector("#timer").innerHTML =
        "Time: " + timeNow(startTime);

      var newStatus = CTX_level.checkCollision(CTX_playerCharge);
      if (
        CTX_level.winbox.inBox(CTX_playerCharge.x, CTX_playerCharge.y) ||
        newStatus == "win"
      ) {
        gameStatus = "win";
      ...
      if (
        outBox(CTX_playerCharge.x, CTX_playerCharge.y, CTX_playerCharge.r) ||
        newStatus == "lose"
      ) {
        gameStatus = "lose";
        CTX_playerCharge = undefined;
...
function updateCamera(pC) {
  if (pC.x < CTX_CAMERA.x + CTX_CAMERA.w * (CTX_CAMERA.th - 0.5))
    //pan left
    CTX_CAMERA.x = Math.max(
      CTX_CAMERA.w * 0.5,
      pC.x + (0.5 - CTX_CAMERA.th) * CTX_CAMERA.w
    );

  if (pC.x > CTX_CAMERA.x - CTX_CAMERA.w * (CTX_CAMERA.th - 0.5))
    //pan right
    CTX_CAMERA.x = Math.min(
      CTX_level.w - CTX_CAMERA.w * 0.5,
      pC.x - (0.5 - CTX_CAMERA.th) * CTX_CAMERA.w
    );

  if (pC.y < CTX_CAMERA.y + CTX_CAMERA.h * (CTX_CAMERA.th - 0.5))
    //pan up
    CTX_CAMERA.y = Math.max(
      CTX_CAMERA.h * 0.5,
      pC.y + (0.5 - CTX_CAMERA.th) * CTX_CAMERA.h
    );

  if (pC.y > CTX_CAMERA.y - CTX_CAMERA.h * (CTX_CAMERA.th - 0.5))
    //pan down
    CTX_CAMERA.y = Math.min(
      CTX_level.h - CTX_CAMERA.h * 0.5,
      pC.y - (0.5 - CTX_CAMERA.th) * CTX_CAMERA.h
    );
}
